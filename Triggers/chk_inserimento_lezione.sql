...
BEGIN 
    ...
    SELECT * INTO dipendente_info FROM dipendente WHERE fk_persona = :NEW.fk_dipendente AND data_di_licenziamento IS NULL;  
    ...
    IF(UPPER(dipendente_info.mansione) != 'ISTRUTTORE' OR dipendente_info.mansione IS NULL) THEN  
        RAISE mansione_exc; 
	ELSIF(dipendente_info.fk_sede_via != :NEW.fk_sala_via AND dipendente_info.fk_sede_civico != :NEW.fk_sala_civico AND dipendente_info.fk_sede_cap != :NEW.fk_sala_cap) THEN
        RAISE sede_dip_exc;
    END IF;
...
EXCEPTION 
    ...
    WHEN mansione_exc THEN   
        RAISE_APPLICATION_ERROR('-20122', 'LA MANSIONE DEL DIPENDENTE È DIVERSA DA ISTRUTTORE: ' || dipendente_info.mansione);  
