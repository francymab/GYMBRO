...
CREATE TABLE Tipologia_turno (   
    ...
    giorno_settimana_turno CHAR(3) CHECK ( UPPER(giorno_settimana_turno) 
                                           IN ( 'LUN', 'MAR', 'MER', 'GIO', 'VEN', 'SAB' ) ) NOT NULL,
    durata_turno           NUMBER(3, 0) CHECK ( durata_turno > 0 ) NOT NULL,   
       
    -- VINCOLI DI CHIAVE PRIMARIA --  
    CONSTRAINT pk_tipologia_turno PRIMARY KEY ( nome_tipologia_turno, giorno_settimana_turno )
);
...
CREATE TABLE Prodotto (   
    ...
    giacenza                NUMBER(4, 0) CHECK ( giacenza > 0 ) NOT NULL
);
...
CREATE TABLE Presenza_dipendente (   
    -- ATTRIBUTI --   
    data_presenza             DATE DEFAULT CURRENT_DATE,
    orario_entrata            TIMESTAMP,
    ...
    fk_tipologia_turno_giorno CHAR(3) NOT NULL,   
   
    -- VINCOLI DI CHIAVE PRIMARIA --   
    CONSTRAINT pk_presenza_dipendente PRIMARY KEY ( data_presenza,
                                                    orario_entrata,
                                                    fk_dipendente ),   
    ...
    CONSTRAINT fk_tipologia_turno_presenza FOREIGN KEY ( fk_tipologia_turno_nome,
                                                         fk_tipologia_turno_giorno )
        REFERENCES Tipologia_turno ( nome_tipologia_turno,
                                     giorno_settimana_turno ) -- on delete default --   
);
...
CREATE TABLE Scontrino(  
    ...
    prezzo_totale NUMBER(5,2) CHECK (prezzo_totale > 0),  
...
CREATE TABLE Sottoscrizione (  
    ...
    fk_sede_civico            VARCHAR2(5) NOT NULL,
    fk_sede_cap               NUMBER(5, 0) NOT NULL,
    data_inizio_sottoscrzione DATE DEFAULT current_date NOT NULL,
...
CREATE TABLE Assegnazione_settimanale ( 
    ...
    fk_tipologia_turno_giorno CHAR(3), 
...
CREATE TABLE Vendita (  
    ...
    fk_prodotto      CHAR(13),
    quantita_venduta NUMBER(3, 0) CHECK ( quantita_venduta BETWEEN 1 AND 5 ) NOT NULL,  
...
CREATE TABLE Compreso(  
    ...
    fk_tipologia_abbonamento_nome VARCHAR2(100),  
    fk_tipologia_abbonamento_durata NUMBER(2,0),  
    	  
    -- VINCOLI DI CHIAVE PRIMARIA --  
    CONSTRAINT pk_compreso PRIMARY KEY(fk_corso, fk_tipologia_abbonamento_nome, fk_tipologia_abbonamento_durata),  
      
    -- VINCOLI DI CHIAVE ESTERNA --  
    CONSTRAINT fk_corso_compreso FOREIGN KEY(fk_corso)   
    	REFERENCES Corso(nome_corso) ON DELETE CASCADE,  
      
    CONSTRAINT fk_tipologia_abbonamento_compreso FOREIGN KEY(fk_tipologia_abbonamento_nome, fk_tipologia_abbonamento_durata)  
